...
`champion_ability_scaling_profile.jsonl` uses compact per-champion fields for model input: `cid`, `champ`, `ab`, `sc_ab`, `sc_ty`, `sc_m`, and `sc_st`, plus compact feature counts such as `p_ap_ab`, `p_t_max_hp_ab`, `e_shield_ab`, `cc_slow_ab`, and `stk_ab`. The `_ab` suffix counts abilities and `_st` counts scaling/effect stages; `stk` comes from the raw `stack_scaling` feature for unique champion growth systems.
